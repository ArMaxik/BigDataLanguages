public class Phone {
    ...
    public Phone(
        ...
        int intercity_calls,
        int intracity_calls
    ) throws InvalidCardNumberException, InvalidPatronymicException {
        if (!_card_number.matches("^[0-9]{4}( [0-9]{4}){3}$")) throw new InvalidCardNumberException(_card_number);
        if (!_patronymic.endsWith("вич")) throw new InvalidPatronymicException(patronymic);
