...
public class House {
    ...
    public House(
        ...
        int lifetime
    ) throws InvalidHouseInfoException {
        if (Stream.of(_id, _number, _square, _floor, _room_count, _lifetime, _street, _building_type).anyMatch(Objects::isNull)) 
            throw new InvalidHouseInfoException(null, null);
        if (square <= 0) throw new InvalidHouseInfoException("_square", Integer.toString(square));
        if (floor <= 0) throw new InvalidHouseInfoException("_floor", Integer.toString(floor));
        if (lifetime <= 0) throw new InvalidHouseInfoException("_lifetime", Integer.toString(lifetime));
